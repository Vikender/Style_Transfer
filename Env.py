...
class CabDriver():
   ...
   #     return state_encod
    ...
    #     """convert the (state-action) into a vector so that it can be fed to the NN. This method converts a given state-action pair into a vector format. Hint: The vector is of size m + t + d + m + m."""
        state_vec=np.zeros(m+t+d+m+m)
        state_vec[m+state[1]]=state[1]
        state_vec[m+t+state[2]]=state[2]
        state_vec[m+t+d+action[0]]=action[0]
        state_vec[m+t+d+m+action[1]]=action[1]
